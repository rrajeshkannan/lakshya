...
def _elevation(*, rolling_3y=None, rolling_5y=None, rolling_7y=None, rolling_10y=None):
    return SimpleNamespace(rolling_3y=rolling_3y, rolling_5y=rolling_5y, rolling_7y=rolling_7y, rolling_10y=rolling_10y)
...
def _purpose(*, horizon_years=10):
    return Purpose(name="Test purpose", capital=500_000, desired_target=1_500_000, horizon_years=horizon_years)
...
def test_achievability_rejects_composition_below_required_elevation():
    ...
    required = required_annual_return(purpose)
    fingerprint = _fingerprint(_elevation(rolling_10y=SimpleNamespace(maximum=required - 0.01)))
    assessment = assess_achievability(purpose, fingerprint, required)
