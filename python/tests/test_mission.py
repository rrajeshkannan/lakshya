...
def test_open_purpose_bypasses_achievability_and_uses_fixed_seven_year_horizon():
    purpose = Purpose(name="Stitch", capital=500_000)
    assert purpose.has_achievability is False
    assert purpose.trajectory_horizon_years == 7


def test_open_purpose_does_not_invent_target_or_deadline():
    purpose = Purpose(name="Kutti", capital=500_000)
