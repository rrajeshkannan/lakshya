...
def test_stream_composition_fingerprints_expands_each_admitted_team():
    ...
    assert all(all(weight > 0 for weight in composition.weights.values()) for composition, _ in results)


def test_stream_composition_fingerprints_preserves_positive_three_member_coordinate_space():
    ...
    assert any(composition.weights == {"A": 0.90, "B": 0.05, "C": 0.05} for composition, _ in results)
    assert all(all(weight > 0 for weight in composition.weights.values()) for composition, _ in results)
