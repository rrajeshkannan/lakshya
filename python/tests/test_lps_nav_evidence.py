...
def test_nav_evidence_store_prepends_new_observations(tmp_path):
    ...
    store = NavEvidenceStore(path)
    store.create(
        isin="INFTEST123", scheme_code=12345, source="mfapi.in",
        nav=initial, retrieved_at="2026-08-04T16:00:00+05:30",
    )
    ...
    store.update(nav=new, retrieved_at="2026-08-06T16:00:00+05:30")

    payload = json.loads(path.read_text(encoding="utf-8"))
    assert payload["artifact_version"] == 2
    assert [o["date"] for o in payload["observations"]] == [
        "2026-08-05", "2026-08-04", "2026-08-03", "2026-08-02", "2026-08-01"
    ]


def test_nav_evidence_store_rejects_observations_not_strictly_newer(tmp_path):
    path = tmp_path / "INFTEST123_nav.json"
    initial = pd.DataFrame({
        "date": pd.to_datetime(["2026-08-03", "2026-08-02"]),
        "nav": [103.0, 102.0],
    })
    store = NavEvidenceStore(path)
    store.create(
    ...
    overlapping = pd.DataFrame({
        "date": pd.to_datetime(["2026-08-04", "2026-08-03"]),
        "nav": [104.0, 999.0],
    })

    with pytest.raises(ValueError, match="strictly newer"):
        store.update(nav=overlapping, retrieved_at="2026-08-05T16:00:00+05:30")


def test_nav_evidence_store_rejects_identity_change(tmp_path):
    path = tmp_path / "INFTEST123_nav.json"
    nav = pd.DataFrame({
        "date": pd.to_datetime(["2026-08-03"]), "nav": [103.0]
    })
    store = NavEvidenceStore(path)
    store.create(
        isin="INFTEST123", scheme_code=12345, source="mfapi.in",
        nav=nav, retrieved_at="2026-08-04T16:00:00+05:30",
    )

    with pytest.raises(ValueError, match="identity"):
        store.update(
            nav=pd.DataFrame({
                "date": pd.to_datetime(["2026-08-04"]), "nav": [104.0]
            }),
...
def test_nav_evidence_can_be_created_from_normalized_history(tmp_path):
    ...
    store.create(
        isin="INFTEST123", scheme_code=12345, source="mfapi.in",
        nav=normalized, retrieved_at="2026-08-17T16:00:00+05:30",
    )
...
def test_nav_evidence_store_returns_no_new_observations_when_history_is_current(tmp_path):
    ...
    new_observations = incoming[incoming["date"] > store.latest_date()]
...
def test_nav_evidence_store_returns_latest_observation_on_or_before_date(tmp_path):
    path = tmp_path / "INFTEST123.json"
    nav = pd.DataFrame({
        "date": pd.to_datetime([
            "2026-08-18", "2026-08-17", "2026-08-14", "2026-08-13", "2026-08-12"
        ]),
    ...
    store.create(
        isin="INFTEST123", scheme_code=12345, source="mfapi.in",
        nav=nav, retrieved_at="2026-08-19T16:00:00+05:30",
    )
...
def test_nav_evidence_store_as_of_returns_same_observation_for_adjacent_missing_day(tmp_path):
    path = tmp_path / "INFTEST123.json"
    nav = pd.DataFrame({
        "date": pd.to_datetime(["2026-08-18", "2026-08-17", "2026-08-14"]),
    ...
    assert store.as_of(pd.Timestamp("2026-08-16")) == (
        pd.Timestamp("2026-08-14"), 109.78
    )


def test_nav_evidence_store_as_of_rejects_date_before_first_observation(tmp_path):
    path = tmp_path / "INFTEST123.json"
    nav = pd.DataFrame({
        "date": pd.to_datetime(["2026-08-14"]), "nav": [109.78]
    })
    store = NavEvidenceStore(path)
    store.create(
        isin="INFTEST123", scheme_code=12345, source="mfapi.in",
