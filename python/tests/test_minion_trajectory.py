...
def test_three_way_path_normalizes_all_paths_to_same_start():
    ...
    assert path.loc[0, "norm_trio"] == 1.0
    assert path.loc[0, "norm_twin_a"] == 1.0
    assert path.loc[0, "norm_twin_b"] == 1.0
    assert path.loc[1, "norm_trio"] == 1.1
    assert path.loc[1, "norm_twin_a"] == 1.1
    assert path.loc[1, "norm_twin_b"] == 1.1
...
def test_residual_detects_minion_present_path_departure():
    ...
    assert path.loc[1, "residual_a_pct_points"] > 0
    assert path.loc[1, "residual_b_pct_points"] < 0
    assert not path.loc[1, "trio_inside_twin_envelope"]
