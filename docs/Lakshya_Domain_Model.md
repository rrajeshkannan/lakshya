...
The current transition implementation reinforces, rather than replaces, the established language:

- a lot is evidence derived from Transactions, not an alternative authoritative ledger;
- availability and lock status are interpretations that must retain source traceability;
- Purpose mapping is explicit and reviewable;
- a transition slice is an analytical/review artifact, not a factual Position;
- reconciliation does not manufacture Positions; external evidence establishes them;
- promotion of Purpose attribution remains a human-controlled act.

No new generic `Action`, `CurrentState`, or optimizer domain object is required merely because the implementation needs intermediate artifacts.
